...
pub fn impl_state_enum(ast: &DeriveInput, en: &DataEnum) -> TokenStream {
    ...
    for (i, variant) in en.variants.iter().enumerate() {
        ...
        match variant.fields {
            ...
            Fields::Unnamed(ref unnamed) => {
                ...
                fields.push(quote!(#field: <#element as amethyst::State<E, #base>>::Storage));
    ...
    quote! {
        ...
        impl<E> amethyst::StateStorage<#base, #base, E> for #storage<E> {
            fn insert(
                &mut self,
                state: #base,
                callback: #callback,
...
pub fn impl_state_struct(ast: &DeriveInput, _: &DataStruct) -> TokenStream {
    ...
    quote! {
        impl<E, T> amethyst::State<E, T> for #base {
            type Storage = amethyst::MapStateStorage<Self, T, E>;
        }
    }
}
