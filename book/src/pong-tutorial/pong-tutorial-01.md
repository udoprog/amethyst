...
We'll be implementing the [`StateHandler`][sc] trait on this struct, which is used by
...
The `StateHandler` already implements a bunch of stuff for us, like the `update` 
...
continue to run until our `StateHandler` returns `Trans::Quit`, or when all states
...
[sc]: https://www.amethyst.rs/doc/master/doc/amethyst/prelude/trait.StateHandler.html
