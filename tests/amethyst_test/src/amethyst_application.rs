...
type FnStateAdd<S, E> = SendBoxFnOnce<'static, (), Box<StateHandler<S, E>>>;
    ...
    E: 'static + Send + Sync,
{
        ...
        T: StateHandler<S, E>,
    {
        self.states.push((
            state,
            SendBoxFnOnce::from(move || Box::new(callback()) as Box<StateHandler<S, E>>),
        ));
...
mod test {
    ...
    fn bundle_build_is_ok() {
        assert!(AmethystApplication::<(), _, _>::blank()
    ...
    fn base_application_can_load_ui() {
        ...
        assert!(AmethystApplication::<(), _, _>::ui_base::<String, String>()
            .do_fn(assertion_fn)
            .run()
            .is_ok());
    }
    ...
    fn with_system_single_runs_system_once() {
        ...
        assert!(AmethystApplication::<&str, _, _>::blank()
    ...
    impl<S, E> StateHandler<S, E> for LoadingState<S> {
    ...
    impl<S, E> StateHandler<S, E> for SwitchState<S> {
